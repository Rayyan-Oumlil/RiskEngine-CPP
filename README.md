...
- **Phase 6, risk measures on non-linear positions:** on a delta-hedged short straddle, delta-normal VaR is 0, delta-gamma normal understates the full-revaluation VaR by 41 %, and adding the implied-vol factor doubles it. A 34-year backtest on frozen FRED data (NASDAQ, VIX, T-bill) puts the linear and quadratic methods in the Basel red zone every year; historical simulation passes coverage on average but fails Christoffersen's independence test (clustered crisis exceptions). Historical stress scenarios (1987, 2008, 2018, 2020) cost up to 11 times the historical 99 % VaR.

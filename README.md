...
- **Phase 3, trees:** CRR, averaged CRR, Leisen-Reimer, BBS and BBS-Richardson, European and American, with delta, gamma and theta read off an extended tree. CRR's error flips sign between even and odd n (−2.00/n against +1.75/n at the money); Leisen-Reimer is the only second-order method (3.6 × 10⁻⁹ at n = 10,000); BBS-Richardson is second order only for n divisible by 4, and early exercise brings every method back to first order. Extension: American exercise by least-squares Monte Carlo (Longstaff-Schwartz), split-sample to keep the regression's look-ahead bias out of the reported standard error, and within 0.7 standard errors of the tree's own reference (`docs/model_risk_report.md` §4.4).
